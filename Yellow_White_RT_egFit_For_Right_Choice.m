function Yellow_White_RT_egFit_For_Right_Choice(data)
...
for k1=1:N1
    ...
    RTSYs=RTS(COLS=='Y');
    Respsy=Responses(COLS=='Y');
    RTSY=RTSYs(Respsy==1);
    RTSWs=RTS(COLS=='W');
    Respsw=Responses(COLS=='W');
    RTSW=RTSWs(Respsw==1);
    [NY,~]=size(RTSY);
    SSY=[SSY;NY];
    [NW,~]=size(RTSW);
...
writetable(Total,'Yellow_White_RT_Comparison_For_Correct_Choices_50-50incl.csv')
end